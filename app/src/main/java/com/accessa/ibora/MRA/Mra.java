            ...
            try {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (cursor != null && cursor.moveToFirst()) {
                        ...
                        int columnIndexTotalHT = cursor.getColumnIndex(DatabaseHelper.TRANSACTION_TOTAL_HT_A);
...
// Convert the doubles to formatted strings with 2 decimal places
                        ...
                        jsondetailedtransacs.put("invoiceCounter", "1");
                        jsondetailedtransacs.put("transactionType", "B2C");
                        jsondetailedtransacs.put("personType", "VATR");
                        jsondetailedtransacs.put("invoiceTypeDesc", "DRN");
                        jsondetailedtransacs.put("currency", "MUR");
                        jsondetailedtransacs.put("invoiceIdentifier", "test2");
                        jsondetailedtransacs.put("invoiceRefIdentifier", "test1");
                        jsondetailedtransacs.put("previousNoteHash", "prevNote");
                        jsondetailedtransacs.put("reasonStated", "return of product");
                        ...
                        buyer.put("name", "James");
                        buyer.put("tan", "12345678");
                        buyer.put("brn", "");
                        buyer.put("businessAdd", "");
                        ...
                        List<Item> itemListFromDatabase = mDatabaseHelper.getTransactionsByStatusAndId(mDatabaseHelper.TRANSACTION_STATUS_IN_PROGRESS, transactionIdInProgress); // Replace with your method to fetch items

// Construct the "itemList" JSON array
                        JSONArray itemList = new JSONArray();

                        for (Item itemFromDatabase : itemListFromDatabase) {
                            JSONObject item = new JSONObject();
                            item.put("itemNo", itemFromDatabase.getItemNo());
                            item.put("taxCode", itemFromDatabase.getTaxCode());
                            item.put("nature", itemFromDatabase.getNature());
                            item.put("currency", itemFromDatabase.getCurrency());
                            item.put("itemCode", itemFromDatabase.getItemCode());
                            item.put("itemDesc", itemFromDatabase.getItemDesc());
                            item.put("quantity", itemFromDatabase.getQuantity());
                            item.put("unitPrice", itemFromDatabase.getUnitPrice());
                            item.put("discount", itemFromDatabase.getDiscount());
                            item.put("amtWoVatCur", itemFromDatabase.getAmtWoVatCur());
                            item.put("amtWoVat", itemFromDatabase.getAmtWoVat());
                            item.put("tds", itemFromDatabase.getTds());
                            item.put("vatAmt", itemFromDatabase.getVatAmt());
                            item.put("totalPrice", itemFromDatabase.getTotalPrice());
                            itemList.put(item);
                        }
...
// If QR code not found, return appropriate message
    ...
    private void returnQRCodeToCallingActivity(String qrCode) {
        Intent resultIntent = new Intent();
        resultIntent.putExtra("QR_CODE", qrCode);
        setResult(Activity.RESULT_OK, resultIntent);
        finish(); // Finish the Mra activity and return to the calling activity
    }
